...
pre_race_results = scrape_race_result(race_urls, requests)

#各レースデータの行名をレースIDに変更
for key in pre_race_results.keys():
    pre_race_results[key].index = [key]*len(pre_race_results[key])

#全データを結合
race_results = pd.concat([pre_race_results[key] for key in pre_race_results.keys()], sort=False)